...
def check_issue_unique():
    ...
    unique_index = {}
    for id, item in collection.get_collection().items():
    ...
    for id, item in unique_index.items():
        if len(item) > 1:
            message = 'Найдено нарушение на борде по правилу 1 задача на 1 человека в определенном статусе'
            for issue in item:
                message = message + '\n%s со статусом %s, исполнитель %s' % (issue.get_key(), issue.get_status().get_name(), issue.get_assignee().get_display_name())
